...
var AppClient = function () {
    ...
    _createClass(AppClient, null, [{
        ...
        value: async function findWays(fromPositionStr, toPositionStr, myStartTimeStr, my_dopTimeMinutes, my_speed, typesStr) {
            ...
            try {
                // Пробуем получить оптимальные пути с сервера.
                ...
                // Иначе выполняем все расчеты на клиенте.
                //console.log(e);
                try {
        ...
        value: async function findCurrentDestinationCoords() {
            if (navigator.geolocation) {
                var getCurrentPosition = async function getCurrentPosition() {
                ...
                var coords = { lat: findedLat, lng: findedLng };
                ...
                if (lastCnownPositionCoordsDescription == null) {
                    var searchingPoint = await _pointsHistoryStorage2.default.tryFindByCoords(coords);
        ...
        value: async function findPointsByOsmGeocodingApi(strReq) {
            try {
                var data = await getJsonFromUrl("https://nominatim.openstreetmap.org/search?q=" + strReq + "&format=json");
                if (data != null && data.length !== 0) {
                    var resultPoints = [];
                    for (var i = 0, n = data.length, currentPoint = data[0]; i < n; currentPoint = data[++i]) {
                        resultPoints.push({
        ...
        value: async function getDesinationDescription(coords) {
            try {
                var findedPoints = await AppClient.findPointsByOsmGeocodingApi(coords.lat + "," + coords.lng);
