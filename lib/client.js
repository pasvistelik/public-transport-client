...
var AppClient = function () {
    ...
    (0, _createClass3.default)(AppClient, null, [{
        ...
        value: function customizeFindedOptimalWaysStart(totalTimePercentValue, totalGoingTimePercentValue, totalTransportChangingCountPercentValue, totalWaitingTimePercentValue, riskPercentValue) {
            if (AppClient.findedOptimalWays != null) {
                ...
                for (var j = 0; j < AppClient.findedOptimalWays.length /* && j < 3*/; j++) {
                    ...
                    for (var i = 0; i < AppClient.findedOptimalWays.length; i++) {
                        if (sortedArr.indexOf(i) === -1) {
                            ...
                            var totalTimeRank = AppClient.minimalTimeSeconds / parseFloat(AppClient.findedOptimalWays[i].totalTimeSeconds) * totalTimePercentValue;
                            var totalGoingTimeRank = AppClient.minimalGoingTimeSeconds / parseFloat(AppClient.findedOptimalWays[i].totalGoingTimeSeconds) * totalGoingTimePercentValue;
                            var totalTransportChangingCountRank = tmpTransportChangingCountEffictivity * totalTransportChangingCountPercentValue;
                            ...
                            var totalWaitingTimeRank = (1 - parseFloat(AppClient.findedOptimalWays[i].totalWaitingTime) / parseFloat(AppClient.findedOptimalWays[i].totalTimeSeconds)) * totalWaitingTimePercentValue;
                            var totalRiskRank = parseFloat(AppClient.findedOptimalWays[i].riskEffectivity) * riskPercentValue;
        ...
        key: "totalWaitingTimePercentValue",
        get: function get() {
